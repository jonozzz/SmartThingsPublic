...
private tvAction(key,deviceNetworkId) {
    ...
    log.debug "6 ${authenticationMessage} ${authenticationMessageLength}"
    ...
    log.debug "7 ${authenticationPacket}"
    ...
    //def x = "${(char)0x00}${(char)appStringLength}${(char)0x00}${appString}"
    def x = "414141"
    def action = new physicalgraph.device.HubAction(x, physicalgraph.device.Protocol.LAN, "C0A80181:D6D8")
    //action.options = [type:"LAN_TYPE_TCPCLIENT"]
    sendHubCommand(action)
    log.debug "9 worked ${x} y4"
    return
